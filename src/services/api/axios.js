...
export const apiGet = (url, { params = {}, otps = {}, qsOtps = {} }) => {
  return api.get(
    ...
    {
      params,
      paramsSerializer: param => qs.stringify(param, { arrayFormat: 'repeat', charset: 'utf-8', ...qsOtps }),
    },
    otps
  )
}

export const apiPost = (url, { params = {}, data = {}, otps = {}, qsOtps = {} }) => {
  const paramString = qs.stringify(params, { arrayFormat: 'repeat', charset: 'utf-8', ...qsOtps });
  const urlPost = paramString ? `${url}?${paramString}` : url
  return api.post(urlPost, data, otps)
}

export const apiPut = (url, { params = {}, data = {}, otps = {}, qsOtps = {} }) => {
  const paramString = qs.stringify(params, { arrayFormat: 'repeat', charset: 'utf-8', ...qsOtps });
  const urlPut = paramString ? `${url}?${paramString}` : url
  return api.put(urlPut, data, otps)
}

export const apiPatch = (url, { params = {}, data = {}, otps = {}, qsOtps = {} }) => {
  const paramString = qs.stringify(params, { arrayFormat: 'repeat', charset: 'utf-8', ...qsOtps });
  const urlPut = paramString ? `${url}?${paramString}` : url
